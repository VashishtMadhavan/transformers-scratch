...
class GroupQueryAttention(nn.Module):
    def __init__(self, input_dim: int, dim: int = 64, num_heads: int = 8, groups: int = 1):
        ...
        self.Q = nn.ModuleList([nn.Linear(input_dim, self.dim) for _ in range(self.num_heads)])
        # Shared K,V for all heads
        self.K = nn.ModuleList([nn.Linear(input_dim, self.dim) for _ in range(self.groups)])
        self.V = nn.ModuleList([nn.Linear(input_dim, self.dim) for _ in range(self.groups)])
        self.W = nn.Linear(num_heads * self.dim, self.dim)
    
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # The input tensor is of shape (batch_size, seq_len, input_dim)
        z_list = []
        for i in range(self.num_heads):
            # Applying attention to each head
            attn = torch.bmm(self.Q[i](x), self.K[i % self.groups](x).transpose(1, 2)) / np.sqrt(self.dim)
            z = torch.bmm(torch.softmax(attn, dim=-1), self.V[i % self.groups](x))
            z_list.append(z)
        z_total = torch.cat(z_list, dim=-1) # tensor is of shape (batch_size, seq_len, num_heads * dim)
        return z_total
